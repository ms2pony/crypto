...
DECL(void, sqrn)
(argElement_1w a, unsigned int times)
{
    ...
    for (it = 0; it < times; it++)
    {
        ...
        A3 = b3;
        A4 = b4;
        ai = SHUF32128(A0, 0x44);
        aj = ADD128(ai, ai); /* 2a0 */
        ak = SHUF32128(A1, 0x44);
        al = ADD128(ak, ak); /* 2a1 */
        ak = BLEND32128(ak, al, 0x3);
        ao = ADD128(ak, ak);
        ...
        c0 = MUL128(b0, ai);
        c0 = ADD128(c0, MUL128(d4, ao));
        c1 = MUL128(b1, aj); /*c1 = ADD128(c1, MUL128(b0, --));*/
        c2 = MUL128(b2, aj);
        c2 = ADD128(c2, MUL128(b1, ak));
        c3 = MUL128(b3, aj);
        c3 = ADD128(c3, MUL128(b2, ae));
        c4 = MUL128(b4, aj);
        c4 = ADD128(c4, MUL128(b3, ao));

        ai = SHUF32128(A2, 0x44);
        aj = ADD128(ai, ai); /* 2a2 */
        ak = SHUF32128(A3, 0x44);
        al = ADD128(ak, ak); /* 2a3 */
        ak = BLEND32128(ak, al, 0xC);
        ao = ADD128(ak, ak);
        ...
        c1 = ADD128(c1, MUL128(d4, aj));
        c1 = ADD128(c1, MUL128(d3, ak));
        ...
        c3 = ADD128(c3, MUL128(d4, ai));
        c3 = ADD128(c3, MUL128(d3, ao));
        /*c4 = ADD128(c4,MUL128(b0,--));*/ c4 = ADD128(c4, MUL128(d4, ae));

        ai = SHUF32128(A1, 0xEE);
        aj = ADD128(ai, ai);
        ak = SHUF32128(A2, 0xEE);
        al = ADD128(ak, ak); /* 2a7 */
        ak = BLEND32128(ak, al, 0x3);
        ao = ADD128(ak, ak);
        ae = SHUF32128(ao, 0x4E);
        ...
        b4 = MUL128(b4, k_19_1);
        b3 = MUL128(b3, k_19_1);
        c0 = ADD128(c0, MUL128(b4, aj));
        c0 = ADD128(c0, MUL128(b3, ao));
        ...
        c4 = ADD128(c4, MUL128(d3, aj));
        c4 = ADD128(c4, MUL128(d2, ak));

        ai = SHUF32128(A3, 0xEE);
        aj = ADD128(ai, ai);
        ak = SHUF32128(A4, 0xEE);
        al = ADD128(ak, ak);
...
DECL(int, cmp)
(argElement_1w a, argElement_1w b)
{
    ALIGN uint8_t a_bytes[SIZE_FP25519];
...
DECL(void, cmv)
(int bit, argElement_1w c, argElement_1w a, argElement_1w b)
{
...
DECL(void, add)
(argElement_1w c, argElement_1w a, argElement_1w b)
{
    __m256i a0 = LOAD(a + 0);
    __m256i a1 = LOAD(a + 1);
    __m256i a2 = LOAD(a + 2);
...
DECL(void, addsub)
(argElement_1w pA, argElement_1w pB)
{
    const __m256i _2P[3] = {
...
DECL(void, copy)
(argElement_1w c, argElement_1w a)
{
    STORE(c + 0, LOAD(a + 0));
    STORE(c + 1, LOAD(a + 1));
    STORE(c + 2, LOAD(a + 2));
...
DECL(void, invsqrt)
(argElement_1w uv_p38, argElement_1w u, argElement_1w v)
{
    ...
    {
        FN(mul)
        (uv_p38, uv_p38, (argElement_1w)sqrt_minus_one); /* (√-1)*B */
    }
}
...
DECL(void, naddsub)
(argElement_1w pC, argElement_1w pD, argElement_1w pA, argElement_1w pB)
{
    const __m256i _2P[3] = {
        SET64(0x7fffffe, 0x3fffffe, 0x3fffffe, 0x7ffffda), /* [ h6, h1, h5, h0] */
        SET64(0x7fffffe, 0x3fffffe, 0x3fffffe, 0x7fffffe), /* [ h8, h3, h7, h2] */
